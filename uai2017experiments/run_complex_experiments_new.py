...
def complex_random_test(seed, randomness, n, mu, sd, independent, vertex_kernel_hop, slope):
    ...
    xxx = ci_test_all(skeleton, U, V, W, vertex_kernel_hop=vertex_kernel_hop, gamma_x=0.5 / sd ** 2,
                      gamma_y=0.5 / sd ** 2, gamma_z=0.5 / sd ** 2, use_median=False)
    return (seed, randomness, n, mu, sd, independent, vertex_kernel_hop, slope, *xxx)


def draw_complex_null():
    columns = ['seed', 'randomness', 'n', 'mu', 'sd', 'independent', 'vertex_kernel_hop', 'slope', 'HSIC', 'dummy1',
               'R-HSIC', 'dummy2', 'RK-HSIC', 'dummy3', 'G-KCIT', 'G-SDCIT']
    df_all = pd.read_csv('new_results/complex_null_hypothesis_vk1.csv', names=columns)
    df_all = pd.melt(df_all,
                     id_vars=['seed', 'randomness', 'n', 'mu', 'sd', 'independent', 'vertex_kernel_hop', 'slope'],
                     value_vars=['HSIC', 'dummy1', 'R-HSIC', 'dummy2', 'RK-HSIC', 'dummy3', 'G-KCIT', 'G-SDCIT'],
                     var_name='method',
                     value_name='p-value')
    ...
    for method, df_2 in df_all.groupby(by=['method']):
        ...
        ax.set_title(method)
        plt.savefig('new_figures/complex_null_{}_vk1.pdf'.format(method), transparent=True, bbox_inches='tight',
                    pad_inches=0.02)
        plt.close()
...
def draw_complex_alternative():
    for postfix in ['', '_vk1']:
        for is_aupc in [False, True]:
            columns = ['seed', 'randomness', 'n', 'mu', 'sd', 'independent', 'vertex_kernel_hop', 'slope', 'HSIC',
                       'dummy1', 'R-HSIC', 'dummy2', 'RK-HSIC', 'dummy3', 'G-KCIT', 'G-SDCIT']
            df_all = pd.read_csv('new_results/complex_alternative_hypothesis{}.csv'.format(postfix), names=columns)
            df_all = pd.melt(df_all,
                             id_vars=['seed', 'randomness', 'n', 'mu', 'sd', 'independent', 'vertex_kernel_hop',
                                      'slope'],
                             value_vars=['HSIC', 'dummy1', 'R-HSIC', 'dummy2', 'RK-HSIC', 'dummy3', 'G-KCIT',
                                         'G-SDCIT'],
                             var_name='method',
                             value_name='p-value')
            ...
            g = sns.factorplot(x='slope', y='p-value', hue='method', col='position', ci=0, data=all_df, size=2.2,
                               ...
                               hue_order=['HSIC', 'R-HSIC', 'RK-HSIC', 'G-KCIT', 'G-SDCIT'], legend=False)
            g.set_xticklabels(step=4)
            g.set_xlabels('linear coefficient')
            if is_aupc:
                g.set_ylabels('AUPC')
            else:
                g.set_ylabels('mean p-values')
            ...
            plt.savefig('new_figures/complex_alternative{}{}.pdf'.format(postfix, '_AUPC' if is_aupc else ''),
    ...
    for vertex_kernel_hop in [1]:
        ...
        if multiprocessing.cpu_count() == 8:
            with Parallel(-1) as parallel:
                for seed in trange(152, 200):
                    outss = parallel(delayed(complex_random_test)(i * 200 + seed, *config) for i, config in
                                     enumerate(alt_configurations))
                    with open('new_results/complex_alternative_hypothesis_vk1.csv', 'a') as f:
                        for outs in outss:
