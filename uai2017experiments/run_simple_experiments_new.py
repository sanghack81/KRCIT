...
from uai2017experiments.run_simple_experiments import generate_values, generate_structure, null_configurations2, \
    alternative_configurations2
    ...
    for is_aupc in [False, True]:
        ...
        for i, (ax, title) in enumerate(zip(g.axes.flat, titles)):
            ...
            if i == 0:
                ...
                ax.legend([], [])
        plt.savefig('new_figures/simple_alternative{}.pdf'.format('_AUPC' if is_aupc else ''), transparent=True,
    ...
    with Parallel(3*multiprocessing.cpu_count()//4) as parallel:
        ...
        if not exists('new_results/simple_null_hypothesis.csv'):
            for seed in trange(20):
                ...
                outss = parallel(
                    delayed(simple_random_test)(i * 20 + seed, mixup, 200, mu, 0.1, True, 2, 0.0) for i, (mixup, mu)
        ...
        if not exists('new_results/simple_alternative_hypothesis.csv'):
            for seed in trange(200):
                ...
                outss = parallel(
                    delayed(simple_random_test)(i * 200 + seed, *config) for i, config in enumerate(configurations))
